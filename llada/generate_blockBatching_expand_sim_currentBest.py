...
Extends V1 with batched block-denoise + unified KV cache:
  ...
  - Similarity-triggered KV refresh (each active branch refreshes its own row)
...
def _select_most_divergent_kv_pair(
    states: List[FusedState],
    unified_cache: list,
    stats_top: dict,
) -> Optional[Tuple[FusedState, FusedState, float, float]]:
    """Return the active branch pair with minimum average KV cosine similarity.

    The scan is layer-by-layer to avoid materializing one giant cache tensor.
    Returns (state_a, state_b, min_similarity, max_distance), or None when
    fewer than two active branches are available.
    """
    ...
    try:
        ...
        n_active = len(active)
        sim_matrix = torch.zeros((n_active, n_active), device=device, dtype=torch.float32)
        num_tensors = 0
        ...
        for k_cache, v_cache in unified_cache:
            for cache_tensor in (k_cache, v_cache):
                ...
                sim_matrix += rows @ rows.transpose(0, 1)
                num_tensors += 1

        if num_tensors == 0:
            return None

        sim_matrix /= num_tensors
        triu_i, triu_j = torch.triu_indices(
            n_active, n_active, offset=1, device=device)
        pair_sims = sim_matrix[triu_i, triu_j]
        min_sim, min_idx = torch.min(pair_sims, dim=0)
        i = int(triu_i[min_idx].item())
        j = int(triu_j[min_idx].item())
        min_similarity = float(min_sim.item())
        max_distance = 1.0 - min_similarity

        stats_top['refresh_last_min_similarity'] = min_similarity
        stats_top['refresh_last_max_distance'] = max_distance
        return active[i], active[j], min_similarity, max_distance
    finally:
...
def similarity_refresh_after_block_denoise(
    states: List[FusedState],
    x_batch: torch.Tensor,
    unified_cache: list,
    model,
    ...
    """Run one row-owned full-sequence refresh if active branch KV has diverged."""
    selected = _select_most_divergent_kv_pair(states, unified_cache, stats_top)
    if selected is None:
        stats_top['refresh_skipped_count'] += 1
        return False

    state_a, state_b, min_similarity, max_distance = selected
    divergence_threshold = 2 * 1.0 / (Lp + gen_length)
    if max_distance <= divergence_threshold:
        stats_top['refresh_skipped_count'] += 1
        logger.debug("similarity_refresh skipped: pair=(%d,%d) min_sim=%.6f dist=%.6f threshold=%.6f",
                     state_a.branch_idx, state_b.branch_idx, min_similarity,
                     max_distance, divergence_threshold)
        return False

    logger.debug("similarity_refresh: pair=(%d,%d) min_sim=%.6f dist=%.6f threshold=%.6f",
                 state_a.branch_idx, state_b.branch_idx, min_similarity,
                 max_distance, divergence_threshold)
...
def run_fused_block_batching_v2(
    ...
    Applies both monkey-patches (RoPE + varlen SDPA) once, then:
      ...
      3. After each block-denoise, similarity-triggered row-owned full refresh
         of all active branches when KV distance exceeds the threshold

    Returns:
        (final_x, total_nfe, stats_dict)

    refresh_block_size is retained for call compatibility; refresh is driven by
    post-denoise KV similarity in this variant.
    """
    ...
    while not all(s.done for s in states):
        ...
        refreshed = similarity_refresh_after_block_denoise(
            states, x_batch, unified_cache, model, temperature, remasking,
            mask_id, threshold, eos_token_id, stats_top, Lp, gen_length)
        if refreshed:
            result = _check_eos_ready(states, x_batch, step, start_time, stats_top, mask_id, 'refresh')
            if result:
                return result
            _advance_all_blocks(states, x_batch, Lp, gen_length, mask_id, stats_top)
