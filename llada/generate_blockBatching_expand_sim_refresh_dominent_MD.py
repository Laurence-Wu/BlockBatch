...
Extends V1 with batched block-denoise + unified KV cache:
  ...
  - Mahalanobis effective-dimension KV refresh (each active branch refreshes its own row)
...
def _measure_kv_mahalanobis_divergence(
    states: List[FusedState],
    unified_cache: list,
    stats_top: dict,
) -> Optional[Tuple[FusedState, float, float, float, float]]:
    """Return Mahalanobis ellipsoid-shell divergence for active KV rows.

    For each layer's K and V cache tensor, each active branch row is flattened
    and normalized to a unit vector z_i. The local covariance is never
    materialized in D x D form. Instead, we eigendecompose the small centered
    Gram matrix G_ij = (z_i - mu)^T (z_j - mu), whose non-zero eigenvalues
    match the covariance up to the constant 1 / m.

    With covariance Sigma = X^T X / m, each branch's Mahalanobis distance is
    M_i = m * sum_a q_ia^2 over positive Gram eigenvectors q_a. The effective
    dimension is the participation ratio, (sum lambda)^2 / sum lambda^2.
    Values are averaged across all K/V tensors.

    Returns (outlier_state, mmax, deff, bound, rank), where bound = deff.
    ...
    try:
        ...
        n_active = len(active)
        maha_accum = torch.zeros(n_active, device=device, dtype=torch.float32)
        deff_accum = torch.zeros((), device=device, dtype=torch.float32)
        rank_accum = 0.0
        num_tensors = 0
        ...
        for k_cache, v_cache in unified_cache:
            for cache_tensor in (k_cache, v_cache):
                ...
                max_eval = torch.clamp(eigvals.max(), min=0.0)
                eig_tol = torch.maximum(
                    torch.tensor(1e-8, device=device, dtype=eigvals.dtype),
                    max_eval * 1e-6)
                positive = eigvals > eig_tol

                if positive.any():
                    pos_eigvals = eigvals[positive].clamp_min(0.0)
                    pos_eigvecs = eigvecs[:, positive]
                    maha_accum += n_active * pos_eigvecs.square().sum(dim=1)
                    eig_sum = pos_eigvals.sum()
                    eig_sq_sum = pos_eigvals.square().sum()
                    deff_accum += eig_sum.square() / (
                        eig_sq_sum + torch.finfo(pos_eigvals.dtype).eps)
                    rank_accum += float(positive.sum().item())
                num_tensors += 1

        if num_tensors == 0:
            return None

        avg_maha = maha_accum / num_tensors
        avg_deff = deff_accum / num_tensors
        avg_rank = rank_accum / num_tensors
        mmax_tensor, outlier_idx_tensor = torch.max(avg_maha, dim=0)
        outlier_idx = int(outlier_idx_tensor.item())
        mmax = float(mmax_tensor.item())
        deff = float(avg_deff.item())
        bound = deff
        outlier = active[outlier_idx]

        stats_top['refresh_last_md_mmax'] = mmax
        stats_top['refresh_last_md_deff'] = deff
        stats_top['refresh_last_md_bound'] = bound
        stats_top['refresh_last_md_rank'] = avg_rank
        stats_top['refresh_last_outlier_branch'] = outlier.branch_idx
        return outlier, mmax, deff, bound, avg_rank
    finally:
...
def should_use_full_batch_forward(
    ...
    """Return True when KV Mahalanobis divergence should switch to full-batch forward."""
    measurement = _measure_kv_mahalanobis_divergence(states, unified_cache, stats_top)
    if measurement is None:
        stats_top['refresh_skipped_count'] += 1
        return False

    outlier, mmax, deff, bound, rank = measurement
    if mmax <= bound:
        stats_top['refresh_skipped_count'] += 1
        logger.debug("full_batch_forward skipped: outlier=%d mmax=%.6f deff=%.6f rank=%.2f bound=%.6f",
                     outlier.branch_idx, mmax, deff, rank, bound)
        return False

    logger.debug("full_batch_forward enabled: outlier=%d mmax=%.6f deff=%.6f rank=%.2f bound=%.6f",
                 outlier.branch_idx, mmax, deff, rank, bound)
    return True
...
def run_fused_block_batching_v2(
    ...
    Applies both monkey-patches (RoPE + varlen SDPA) once, then:
      ...
      3. When a KV row leaves the Mahalanobis ellipsoid shell, switch to row-owned
    ...
    forward switch is driven by Mahalanobis effective-dimension divergence in this variant.
    """
    ...
    stats_top: dict = {
        ...
        'refresh_last_md_mmax': 0.0,
        'refresh_last_md_deff': 0.0,
        'refresh_last_md_bound': 0.0,
        'refresh_last_md_rank': 0.0,
