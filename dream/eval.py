...
class Dream(LM):
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code, cache_dir=None):
        self.model = (
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        # decode
        self._last_tokens_generated = int(
            (generation_ids.sequences[0][prompt_ids.shape[1] :] != self.tokenizer.eos_token_id)
            .sum()
            .item()
        )
        self.generated_token_num += self._last_tokens_generated
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            if self.save_dir is not None:
                with open(save_path, 'a', encoding='utf-8') as f:
                    for r in responses:
                        rec = {'answer': r, 'nfe': self._last_nfe,
                               'wall_s': round(self._last_wall, 2),
                               'block_length': self.block_length,
                               'tokens_generated': getattr(self, '_last_tokens_generated', None),
                               'generation_policy': 'confidence_dual_cache_diffusion_generate',
    ...
    def loglikelihood(self, requests: List[Instance]) -> List[Tuple[float, bool]]:
        ...
        with torch.no_grad():
            for elem in tqdm(ds, desc="Computing likelihood..."):
                prefix = elem["prefix"]
                target = elem["target"]
                # Accumulate masked-token log likelihoods over the response span.
                if self.nll_type == 'mc':
