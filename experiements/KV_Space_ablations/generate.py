...
try:
    from kv_space_hooks import EVENT_BLOCK, EVENT_FINAL, EVENT_FULL
...
def _record_official_kv(kv_hook, event_type, cache, *, event_step, block_length,
                        sample_id="sample_0", nfe=None, extra=None):
...
def generate_with_dual_cache_hooked(
    model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    remasking="low_confidence", mask_id=126336, threshold=None, factor=None,
    kv_hook=None, sample_id="sample_0"
):
    """Official LLaDA dual-cache generation with read-only KV logging.

    This mirrors generate_with_dual_cache exactly, except that cache tensors are
    detached and logged after existing full-sequence and block-denoise forwards.
    """
    B = prompt.shape[0]
    ...
    for nb in range(num_blocks):
        ...
        _record_official_kv(
            ...
            sample_id=sample_id,
            nfe=nfe,
            extra={"num_block": nb, "block_start": s, "block_end": e},
        )
        ...
        for i in range(1, steps_per_block):
            ...
            _record_official_kv(
                ...
                sample_id=sample_id,
                nfe=nfe + 1,
                extra={"num_block": nb, "block_start": s, "block_end": e, "inner_step": i},
            )
