...
Extends V1 with batched block-denoise + unified KV cache:
  ...
  - Threshold KV refresh (all active branches → 1 batched full-sequence forward)
...
Semantics match the blockSize_combo infra: each x_batch/cache row is one
independent block-size state. After the identical initial setup, batching
changes only execution shape; tokens and KV rows move between branches only
through the CnC policy.
...
try:
...
except ImportError:  # pragma: no cover - allows importing this file outside the experiment folder.
    KVSpaceLogger = None
    ...
    def state_branch_indices(states):
        return [int(s.branch_idx) for s in states]

    def state_block_sizes(states):
...
def _record_kv_event(
    ...
    *,
    ...
    """Read-only KV hook. It must never mutate generation state."""
    if kv_hook is None or cache is None:
        return
    kv_hook.record_cache(
        event_type,
        cache,
        event_step=event_step,
        branch_indices=state_branch_indices(states),
        block_sizes=state_block_sizes(states),
        sample_id=sample_id,
        nfe=nfe,
        extra=extra,
    )
...
SYNC_THRESHOLD = 8
CONF_THRESHOLD = 0.5
REFRESH_BLOCK_SIZE = 32  # Match blockSize_combo's KV refresh granularity.
...
class FusedState:
    ...
    # KV refresh state. unified_cache stores the actual row-owned cache tensors.
    needs_kv_refresh: bool = False
    blocks_since_refresh: int = 0

    # Per-position top-1 softmax probability from the most recent forward.
    cached_prob: Optional[torch.Tensor] = None
...
def _update_cached_prob(state: FusedState, probs_gpu: torch.Tensor, offset: int) -> None:
    """Update per-position top-1 probabilities from a GPU probability tensor."""
    if state.cached_prob is None:
        state.cached_prob = torch.zeros(state.x.shape[1])
    state.cached_prob[offset:offset + probs_gpu.shape[0]] = (
        probs_gpu.max(dim=-1).values.cpu())
...
def advance_block(state: FusedState, Lp: int, gen_length: int, mask_id: int,
                  refresh_block_size: int = REFRESH_BLOCK_SIZE) -> None:
    """Advance after a full block and flag per-row KV refresh at the threshold."""
    update_block_window(state, Lp, gen_length, mask_id)
    if not state.done:
        state.blocks_since_refresh += 1
        if state.blocks_since_refresh * state.block_size >= refresh_block_size:
            state.needs_kv_refresh = True
...
def _broadcast_kv(past_key_values, N: int) -> list:
    ...
    _assert_cache_rows_separate(cache, N)
    return cache


def _active_states(states: List[FusedState]) -> List[FusedState]:
...
def _branch_index_tensor(states: List[FusedState],
                         device: torch.device) -> Tuple[List[int], torch.Tensor]:
    branch_indices = [s.branch_idx for s in states]
    return branch_indices, torch.tensor(branch_indices, dtype=torch.long, device=device)


def _assert_cache_rows_separate(cache: list, expected_batch: int) -> None:
    """Catch accidental expanded/shared-row KV layouts early."""
    for layer_idx, (k_cache, v_cache) in enumerate(cache):
        assert k_cache.shape[0] == expected_batch, (
            f"layer {layer_idx} K batch {k_cache.shape[0]} != {expected_batch}")
        assert v_cache.shape[0] == expected_batch, (
            f"layer {layer_idx} V batch {v_cache.shape[0]} != {expected_batch}")
        if expected_batch <= 1:
            continue
        k_ptrs = {k_cache[i].data_ptr() for i in range(expected_batch)}
        v_ptrs = {v_cache[i].data_ptr() for i in range(expected_batch)}
        assert len(k_ptrs) == expected_batch, f"layer {layer_idx} K rows alias"
        assert len(v_ptrs) == expected_batch, f"layer {layer_idx} V rows alias"


def _select_cache_rows(unified_cache: list, branch_index_tensor: torch.Tensor) -> list:
    """Gather row-owned KV caches into a compact execution batch."""
    selected = [
        (
            k.index_select(0, branch_index_tensor).contiguous(),
            v.index_select(0, branch_index_tensor).contiguous(),
        )
        for k, v in unified_cache
    ]
    _assert_cache_rows_separate(selected, branch_index_tensor.numel())
    return selected
...
def _copy_refreshed_cache_rows(refreshed_cache: list, unified_cache: list,
    ...
    _assert_cache_rows_separate(unified_cache, unified_cache[0][0].shape[0])


def _copy_cache_row(unified_cache: list, dst_idx: int, src_idx: int) -> None:
    for k_cache, v_cache in unified_cache:
        k_src = k_cache[src_idx].clone()
        v_src = v_cache[src_idx].clone()
        k_cache[dst_idx].copy_(k_src)
        v_cache[dst_idx].copy_(v_src)
    _assert_cache_rows_separate(unified_cache, unified_cache[0][0].shape[0])


def _apply_full_sequence_logits_current_block(
    state: FusedState,
...
def _apply_block_logits(
    ...
    """Apply block-denoise logits to one branch, including EOS detection."""
    probs_blk = F.softmax(logits_blk.float(), dim=-1).squeeze(0)
    _update_cached_prob(state, probs_blk, offset=state.block_start)
...
def batched_full_seq_forward(
    ...
    """Full-sequence forward with broadcast init and row-owned batched refresh.

    Mirrors blockSize_combo.full_sequence_forward: rebuild full-sequence KV,
    refresh full-row probability metadata, and unmask only the state's current
    block. EOS detection remains a block-denoise responsibility.

    is_refresh=False (init):
      - Full forward on x_batch[0:1] (Lp+gen_length tokens); builds KV from scratch.
      - Broadcasts the identical initial KV to all branch rows.
    is_refresh=True (threshold refresh):
      - Passes every active branch's full sequence as one batch.
      - Copies each refreshed cache row back only to its original branch.

    Returns the new/updated unified_cache, or None if all states are done.
    """
    active = _active_states(states)
    if not active:
        return None
    ...
    if is_refresh:
        ...
        branch_indices, branch_index_tensor = _branch_index_tensor(active, x_batch.device)
    ...
    logger.debug("batched_%s_fwd: branches=%s", log_label, [s.branch_idx for s in active])
    ...
    _record_kv_event(
        kv_hook,
        EVENT_REFRESH if is_refresh else EVENT_FULL,
        new_cache,
        active if is_refresh else states,
        event_step=event_step,
        sample_id=sample_id,
        nfe=None,
        extra={"is_refresh": bool(is_refresh), "phase": log_label},
    )
    ...
    for local_idx, state in enumerate(active):
        ...
        state.blocks_since_refresh = 0
        state.needs_kv_refresh = False

        logits_i = out.logits[local_idx:local_idx + 1] if is_refresh else out.logits
        _apply_full_sequence_logits_current_block(
            state, x_batch, logits_i, mask_id, temperature, remasking, threshold)

        logger.debug("  %s branch %d (bs=%d): nfe_full=%d decoded=%d",
                     log_label, bi, state.block_size, state.nfe_full, state.tokens_decoded)
...
def batched_block_denoise(
    ...
    Sends only branches whose current block still contains MASK tokens. The
    selected KV rows form a compact execution cache and are written back by
    branch id, so full/done rows stay untouched.
    """
    active = _active_states(states)
    denoise_states = [
        s for s in active
        if (x_batch[s.branch_idx, s.block_start:s.block_end] == mask_id).sum() > 0
    ]
    if not denoise_states:
        return

    stats_top.setdefault('block_denoise_cache_select_time', 0.0)
    stats_top.setdefault('block_denoise_cache_writeback_time', 0.0)
    stats_top.setdefault('block_denoise_compact_batch_rows', 0)
    stats_top.setdefault('block_denoise_compact_batch_tokens', 0)

    # ── pre: padding / position_ids setup ───────────────────────────────
    device = x_batch.device
    t_pre = _timed_now(device)
    B = len(denoise_states)
    L = x_batch.shape[1]
    max_q_len = max(s.block_end - s.block_start for s in denoise_states)
    branch_indices, branch_index_tensor = _branch_index_tensor(denoise_states, x_batch.device)
    compact_cache = branch_indices != list(range(len(states)))

    x_padded = torch.full((B, max_q_len), mask_id, dtype=torch.long, device=device)
    replace_position = torch.zeros((B, L), dtype=torch.bool, device=device)
    position_ids = torch.zeros((B, max_q_len), dtype=torch.long, device=device)

    for local_idx, state in enumerate(denoise_states):
        length = state.block_end - state.block_start
        x_padded[local_idx, :length] = x_batch[state.branch_idx, state.block_start:state.block_end]
        replace_position[local_idx, state.block_start:state.block_end] = True
        position_ids[local_idx, :length] = torch.arange(
        ...
        position_ids[local_idx, length:] = state.block_end - 1

    t_cache_select = _timed_now(device)
    cache_for_forward = (
        _select_cache_rows(unified_cache, branch_index_tensor)
        if compact_cache else unified_cache
    )
    stats_top['block_denoise_cache_select_time'] += _timed_elapsed(t_cache_select, device)
    ...
    # With B=1 (or all q_lens equal, e.g. BLOCK_SIZES=[128]) the padded tensor
    ...
    q_lens = [s.block_end - s.block_start for s in denoise_states]
    varlen_beneficial = len(q_lens) > 1 and max(q_lens) > min(q_lens)
    _VARLEN_Q_LENS = q_lens if varlen_beneficial else None

    logger.debug("batched_block_denoise: active=%s max_q_len=%d total_q=%d varlen=%s compact=%s",
                 [s.branch_idx for s in denoise_states], max_q_len, sum(q_lens),
                 varlen_beneficial, compact_cache)
    ...
    if _PROFILE_LAYERS:
        _reset_layer_timing()
    try:
        out = model(x_padded, past_key_values=cache_for_forward, use_cache=True,
    ...
    if compact_cache:
        t_cache_write = _timed_now(device)
        updated_cache = out.past_key_values if out.past_key_values is not None else cache_for_forward
        _copy_refreshed_cache_rows(updated_cache, unified_cache, branch_indices)
        stats_top['block_denoise_cache_writeback_time'] += _timed_elapsed(t_cache_write, device)
    ...
    _record_kv_event(
        ...
        nfe=None,
        extra={
            "active_branch_indices": branch_indices,
            "q_lens": q_lens,
            "varlen": bool(varlen_beneficial),
            "compact_cache": bool(compact_cache),
        },
    )
    ...
    stats_top['block_denoise_compact_batch_rows'] += B
    stats_top['block_denoise_compact_batch_tokens'] += sum(q_lens)

    # ── post: logit / unmask per branch ──────────────────────────────────
    t_post = _timed_now(device)
    per_branch_dt = dt / len(denoise_states)

    for local_idx, state in enumerate(denoise_states):
        bi = state.branch_idx
        length = state.block_end - state.block_start
        logits_j = out.logits[local_idx:local_idx + 1, :length]  # (1, block_size, vocab)

        state.nfe_block += 1
        state.total_forward_time += per_branch_dt

        _apply_block_logits(state, x_batch, logits_j, mask_id, temperature,
                            remasking, threshold, eos_token_id)
...
def _sync_branch_to_leader(s: FusedState, leader: FusedState,
    ...
    if unified_cache is not None:
        _copy_cache_row(unified_cache, s.branch_idx, leader.branch_idx)
...
def _merge_states(dst: FusedState, src_list: List[FusedState],
                  ...
                  refresh_block_size: int = REFRESH_BLOCK_SIZE) -> int:
    """Merge best-confidence tokens from compatible sources into dst.

    Matches cnc_merge_replenish_CrossStateBest: scan the union of compatible
    source current block windows and fill dst positions only when dst's own
    token_prob_map is confident in a token offered by those sources.
    """
    ...
    for src in src_list:
        if src.done:
            continue
        for pos in range(src.block_start, src.block_end):
            if pos in seen:
                continue
            seen.add(pos)
    ...
    if changed and not dst.done:
        if (dst.x[0, dst.block_start:dst.block_end] == mask_id).sum() == 0:
            advance_block(dst, Lp, gen_length, mask_id, refresh_block_size)

    return tokens_filled


class MergeReplenishPolicy_CrossStateBest:
    """Cross-state best-token merge + threshold sync policy."""
    ...
    def __init__(self, Lp: int, gen_length: int, mask_id: int,
        ...
        self._refresh_block_size = refresh_block_size

    def compare_and_copy(self, states: List[FusedState],
                         x_batch: torch.Tensor,
                         unified_cache: Optional[list] = None) -> None:
        """Run merge + sync.
...
FusedCrossStateBestPolicy = MergeReplenishPolicy_CrossStateBest
...
def build_result_v2(winner: FusedState, all_states: List[FusedState],
    ...
    nfe_init = winner.nfe_init
    nfe_block = winner.nfe_block
    nfe_refresh = winner.nfe_refresh
    total_nfe = nfe_init + nfe_block + nfe_refresh
    aggregate_nfe_init = sum(s.nfe_init for s in all_states)
    aggregate_nfe_block = sum(s.nfe_block for s in all_states)
    aggregate_nfe_refresh = sum(s.nfe_refresh for s in all_states)
    stats = {
        ...
        'aggregate_nfe_init': aggregate_nfe_init,
        'aggregate_nfe_block': aggregate_nfe_block,
        'aggregate_nfe_refresh': aggregate_nfe_refresh,
        'aggregate_total_nfe': aggregate_nfe_init + aggregate_nfe_block + aggregate_nfe_refresh,
        'nfe_per_block_size': {
            s.block_size: {
                'nfe_init': s.nfe_init,
                'nfe_block': s.nfe_block,
                'nfe_refresh': s.nfe_refresh,
                'nfe_full': s.nfe_full,
        ...
        'tokens_per_block_size': {
            s.block_size: x_batch[s.branch_idx:s.branch_idx + 1] for s in all_states
        },
        'kv_cache_row_per_block_size': {s.block_size: s.branch_idx for s in all_states},
...
def _advance_all_blocks(states: List[FusedState], x_batch: torch.Tensor,
    ...
    for s in states:
        if not s.done and (x_batch[s.branch_idx, s.block_start:s.block_end] == mask_id).sum() == 0:
            advance_block(s, Lp, gen_length, mask_id, refresh_block_size)
    stats_top['advance_block_time'] += _timed_elapsed(t, x_batch.device)


def batched_sync_kv_refresh(
    states: List[FusedState],
    x_batch: torch.Tensor,
    unified_cache: list,
    model,
    ...
    """When any active row hits refresh threshold, refresh all active rows."""
    if not any(s.needs_kv_refresh and not s.done for s in states):
        return unified_cache

    t_refresh = _timed_now(x_batch.device)
    new_cache = batched_full_seq_forward(
        ...
        kv_hook=kv_hook, event_step=event_step, sample_id=sample_id)
    stats_top['refresh_sync_time'] += _timed_elapsed(t_refresh, x_batch.device)
    return unified_cache if new_cache is None else new_cache
...
def _check_eos_ready(states: List[FusedState], x_batch: torch.Tensor,
    ...
    _record_kv_event(
        ...
        event_step=step,
        sample_id=sample_id,
        nfe=winner.nfe_init + winner.nfe_block + winner.nfe_refresh,
        extra={"exit_reason": f"eos_ready_{label}", "winner_branch": winner.branch_idx},
    )
...
def run_fused_block_batching_v2(
    ...
    Applies both monkey-patches (RoPE + varlen SDPA) once, then:
      ...
      2. Per-row block advance + CrossStateBest merge/sync policy
      3. Batched full-sequence refresh when any active row reaches the threshold
      4. Main loop: batched varlen block-denoise using the cached K/V
    ...
    eos_token_id = model.config.eos_token_id if hasattr(model.config, 'eos_token_id') else 2
    ...
    policy = FusedCrossStateBestPolicy(
        Lp=Lp, gen_length=gen_length, mask_id=mask_id,
    ...
    stats_top: dict = {
        ...
        'block_denoise_cache_select_time': 0.0,
        'block_denoise_cache_writeback_time': 0.0,
        'block_denoise_compact_batch_rows': 0,
        'block_denoise_compact_batch_tokens': 0,
        'policy_time': 0.0,
    ...
    unified_cache = batched_full_seq_forward(
        states, x_batch, model, temperature, remasking,
        mask_id, threshold, eos_token_id, stats_top, Lp, gen_length,
        kv_hook=kv_hook, event_step=0, sample_id=sample_id)

    _advance_all_blocks(states, x_batch, Lp, gen_length, mask_id,
                        stats_top, refresh_block_size)

    t_pol = _timed_now(x_batch.device)
    _record_kv_event(
        kv_hook, EVENT_POLICY_PRE, unified_cache, states,
        event_step=0, sample_id=sample_id,
        extra={"phase": "post_init_pre_policy"},
    )
    policy.compare_and_copy(states, x_batch, unified_cache)
    _record_kv_event(
        kv_hook, EVENT_POLICY_POST, unified_cache, states,
        event_step=0, sample_id=sample_id,
        extra={"phase": "post_init_post_policy"},
    )
    stats_top['policy_time'] += _timed_elapsed(t_pol, x_batch.device)

    unified_cache = batched_sync_kv_refresh(
        states, x_batch, unified_cache, model, temperature, remasking,
        mask_id, threshold, eos_token_id, stats_top, Lp, gen_length,
        kv_hook=kv_hook, event_step=0, sample_id=sample_id)
    ...
    while not all(s.done for s in states):
        ...
        batched_block_denoise(states, x_batch, unified_cache, model,
                              ...
                              kv_hook=kv_hook, event_step=step, sample_id=sample_id)

        result = _check_eos_ready(
            states, x_batch, step, start_time, stats_top, mask_id, 'block_denoise',
            unified_cache=unified_cache, kv_hook=kv_hook, sample_id=sample_id)
        if result:
            return result
        _advance_all_blocks(states, x_batch, Lp, gen_length, mask_id,
                            stats_top, refresh_block_size)
        ...
        _record_kv_event(
            kv_hook, EVENT_POLICY_PRE, unified_cache, states,
            event_step=step, sample_id=sample_id,
            extra={"phase": "main_loop_pre_policy"},
        )
        policy.compare_and_copy(states, x_batch, unified_cache)
        _record_kv_event(
            kv_hook, EVENT_POLICY_POST, unified_cache, states,
            event_step=step, sample_id=sample_id,
            extra={"phase": "main_loop_post_policy"},
        )
        stats_top['policy_time'] += _timed_elapsed(t_pol, x_batch.device)
        ...
        for s in states:
            assert s.x.shape[1] == Lp + gen_length
            assert 0 <= s.progress <= gen_length
            if not s.done:
                assert s.block_start < s.block_end
    ...
    _record_kv_event(
        ...
        event_step=step,
        sample_id=sample_id,
        nfe=best.nfe_init + best.nfe_block + best.nfe_refresh,
        extra={"exit_reason": "natural", "winner_branch": best.branch_idx},
    )
